...
void eeprom_read_config(void)
{
	...
	if(config.test_mode == 0xFF)
	{
		...
		config.interval_of_test_s = 120;
		config.time_to_wait_prompt_s = 15;
		config.time_to_wait_connect_s = 25;
		config.time_to_wait_send_ok_s = 25;
...
//*******************************************************************************************************************
// если источником перезагрузки был сброс питания, то делаем соответствующую запись в журнале событий.
